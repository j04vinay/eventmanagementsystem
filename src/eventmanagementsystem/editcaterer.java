...
public class editcaterer extends JFrame implements ActionListener{
    String s[] = new String[]{"id","name","contactId","price","Budget"};
    JTextField t2,t3,t4,t5;
    JButton b1,b2,b3;
    Choice c1;
    editcaterer(){
        JLabel l10 = new JLabel("EDIT CATERER");
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b1){
            ...
            String sql = "update caterer set name='"+s2+"', contactId='"+s3+"', price='"+s4+"', budget='"+s5+"' where id='"+s1+"'";
            try{
            ...
            try{
            conn c=new conn();
            String sql="select * from caterer where id='"+c1.getSelectedItem()+"'";
            ResultSet rs=c.s.executeQuery(sql);
            while(rs.next()){
                t2.setText(rs.getString("name"));
                t3.setText(rs.getString("contactId"));
                t4.setText(rs.getString("price"));
                t5.setText(rs.getString("budget"));
            }
            }catch(Exception e){}
        }
    }
